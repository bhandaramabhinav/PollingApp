/*
Component :                             A Web Api controller that invokes the methods defined in 'ActivityDTORepository.cs' to serve the client requests in performing 'read' operations on the 'Activity' table of our project's database (DSE)
...
Reason for component existence:         To serve the requests of the clients in performing 'read' operations on the 'Activity' table 
Component usage of data structures, algorithms and control(if any): 
    The component contains the below methods:
        'GetAllPolls()', 'GetPoll(int id)'
...
namespace WhatsUrSay.Controllers
{
    public class ActivityDTOController : ApiController
    {
        ...
        //Purpose: Invokes 'GetPolls()' method of PollRepository.cs that returns all the records of type 'poll' from the 'Activity' table
        //Input: None
        ...
        //Purpose: Invokes 'GetPoll(int id)' method of PollRepository.cs that returns a record from the 'Activity' table whose key is 'id'
